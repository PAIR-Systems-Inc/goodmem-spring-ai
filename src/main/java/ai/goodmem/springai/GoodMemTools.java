...
/**
 * Spring AI plugin for GoodMem, the retrieval-augmented generation (RAG) memory backend
 * for AI agents. Each method is annotated with {@link Tool} so it can be registered with
 * a Spring AI {@code ChatClient} or wrapped in a {@code MethodToolCallbackProvider} and
 * exposed to any Spring AI agent.
 *
 * <p>
 * All tool methods catch {@link GoodMemClientException} and return a clean result map
 * containing {@code success=false} and a human-readable {@code error} field, so AI models
 * receive an actionable message instead of a stack trace.
 *
 * <p>
